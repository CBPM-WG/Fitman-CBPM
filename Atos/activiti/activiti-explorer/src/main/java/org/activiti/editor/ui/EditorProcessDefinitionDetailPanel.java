...
public class EditorProcessDefinitionDetailPanel extends DetailPanel {
  ...
  protected void initActions() {
    ...
    actionSelect.addItem(i18nManager.getMessage(Messages.PROCESS_EXPORT_COMPEL));
    ...
    actionSelect.addListener(new ValueChangeListener() {
      ...
      public void valueChange(ValueChangeEvent event) {
        if (i18nManager.getMessage(Messages.PROCESS_COPY).equals(event.getProperty().getValue())) {
          ...
          exportModel();
        } else if (i18nManager.getMessage(Messages.PROCESS_EXPORT_COMPEL).equals(event.getProperty().getValue())) {
          exportModelCompel();
        }
      }
    });
  ...
  protected void exportModelCompel() {
	      ...
	      private static final long serialVersionUID = 1L;
	        ...
	        public DownloadStream getStream() {
	          ...
	          try {
	            ...
	            //COMPEL Introduce automatically the WS bindings.
	            //TODO We have to avoid converting again byte[] to String and the String to byte[]
//	            String bpmnCompel = new String(bpmnBytes);
//	        	BPMN20Manager bpmnMan = new BPMN20Manager(bpmnCompel);
...
//	        	String result = bpmnMan.getFinalDocumentWriter().toString();
		        ...
		        PostMethod post = new PostMethod(CompelUtil.BPMNSERVICE_ENDPOINT);
...
//		        HttpClient client2 = new HttpClient(params);
	        
		        post.setRequestEntity(new StringRequestEntity(bpmnCompel,"text/xml",null));
		        
		        int status = client.executeMethod(post); 
		        String response = post.getResponseBodyAsString();
	            
	            //TODO We add the original xml with the final parser. We have to remove it
		        //
	            ...
	            ByteArrayInputStream in = new ByteArrayInputStream(finalResponse.getBytes());
