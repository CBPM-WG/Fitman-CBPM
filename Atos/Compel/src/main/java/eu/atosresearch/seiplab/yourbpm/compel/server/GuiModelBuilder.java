...
import eu.atosresearch.seiplab.yourbpm.compel.client.AttributeNode;
import eu.atosresearch.seiplab.yourbpm.compel.client.AttributeValueNode;
import eu.atosresearch.seiplab.yourbpm.compel.client.AxiomNode;
import eu.atosresearch.seiplab.yourbpm.compel.client.ConceptNode;
import eu.atosresearch.seiplab.yourbpm.compel.client.InstanceNode;
import eu.atosresearch.seiplab.yourbpm.compel.client.ModelNode;
import eu.atosresearch.seiplab.yourbpm.compel.client.RelationInstanceNode;
import eu.atosresearch.seiplab.yourbpm.compel.client.RelationNode;
import eu.atosresearch.seiplab.yourbpm.compel.shared.OntologyNode;


public class GuiModelBuilder {
	private static Logger logger = LoggerFactory.getLogger(GuiModelBuilder.class);
    ...
    public static OntologyNode buildFromRDF(String data) throws Exception {
        ...
        try {
        	...
        	ontoModel.read(new ByteArrayInputStream(data.getBytes("UTF-8")), null);
        }
        catch(Throwable ex) {
        ...
        if (false == ontoModel.listClasses().hasNext()) {
        	...
        	for(ExtendedIterator<com.hp.hpl.jena.ontology.Ontology> ontoIt = ontoModel.listOntologies(); ontoIt.hasNext();) {        	
        		...
        		if (ontoUri != null && ontoUri.trim().length() > 0) {
        			ontoID = ontoUri;
        			break;
        		}
        	}
    ...
    private static ModelNode processOntResource(OntResource resource) {
    	...
    	if (resource instanceof Individual) {
            ...
            InstanceNode instNode = new InstanceNode(getLabel(instance.getURI()), instance.getURI());
        ...
        if (resource instanceof OntProperty) {
        	...
        	if (prop.getRange() != null && prop.getRange().getLocalName() != null) {
        		label += " [" + prop.getRange().getLocalName() + "]";
        	}
        	String propertyRange = null;
        	if (prop.getRange() != null && prop.getRange().getURI() != null) {
        		propertyRange = prop.getRange().getURI();
        	}
        	
            AttributeNode attr = 
                new AttributeNode(label, 
                		resource.getURI(),
                		propertyRange != null && propertyRange.startsWith("http://www.w3.org/2001/XMLSchema#"));
            if (propertyRange != null) {
           	    attr.setRange(propertyRange);
                attr.setTooltip("Attribute range: " + propertyRange);
            }
            return attr;
        }
    	}
    	catch(Throwable err) {
    		logger.error("Error processing ontology resource", err);
    	}
        return null;
    }
